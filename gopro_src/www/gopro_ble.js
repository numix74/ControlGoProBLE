    ...
    async requestDevice(options) {
        if (this.isAndroid) {
            ...
            try {
                ...
                // FEA6 (Contrôle) et FE20 (Découverte)
                const device = await BluetoothLe.requestDevice({
                    services: [
                        GOPRO_SERVICE_UUID.toLowerCase(),
                        "0000fe20-0000-1000-8000-00805f9b34fb"
                    ]
                });

                const rawId = device.deviceId;
                const trimmedId = rawId.trim().toUpperCase();
                console.log("[Bridge] Appareil sélectionné:", device.name, "| ID:", trimmedId);

                const gattMock = {
                    connected: false,
                    connect: async function () {
                        console.log("[Bridge] Délai de 500ms avant connexion à", trimmedId);
                        await new Promise(r => setTimeout(r, 500));
                        try {
                            await BluetoothLe.connect({ deviceId: trimmedId });
                            console.log("[Bridge] Connecté nativement à", trimmedId, ". Forçage découverte services 1.5s...");

                            // Certains téléphones ont besoin d'une découverte explicite pour peupler le cache
                            try {
                                await BluetoothLe.getServices({ deviceId: trimmedId });
                            } catch (e) { console.warn("[Bridge] getServices a échoué (souvent normal si déjà en cache):", e); }

                            await new Promise(r => setTimeout(r, 1500));
                            this.connected = true;
                            return this;
                        } catch (error) {
                            console.error("[Bridge] Erreur connexion native:", error);
                            this.connected = false;
                            throw error;
                        }
                    },
                    disconnect: async function () {
                        console.log("[Bridge] Déconnexion demandée pour", trimmedId);
                        try {
                            await BluetoothLe.disconnect({ deviceId: trimmedId });
                        } catch (e) { console.warn("[Bridge] Erreur lors de disconnect:", e); }
                        this.connected = false;
                    },
                    getPrimaryService: async (uuid) => {
                        console.log("[Bridge] Obtention service:", uuid);
                        return {
                            getCharacteristic: async (charUuid) => {
                                if (VERBOSE_LOGGING) console.log("[Bridge] Obtention char:", charUuid);
                                return {
                                    uuid: charUuid,
                                    startNotifications: async () => {
                                        if (VERBOSE_LOGGING) console.log("[Bridge] StartNotif sur:", charUuid);
                ...
                console.error("Erreur requestDevice Android:", e);
                throw e;
            }
        } else {
...
class GATTQueue {
    ...
    async enqueue(operation) {
        return new Promise((resolve, reject) => {
            if (VERBOSE_LOGGING) console.log("[Queue] Nouvel élément ajouté.");
    ...
    async process() {
        ...
        const timeoutId = setTimeout(() => {
            console.error("[Queue] TIMEOUT atteint !");
            this.busy = false;
            reject(new Error("GATT Timeout"));
            this.process();
        }, this.operationTimeout);

        try {
            if (VERBOSE_LOGGING) console.log("[Queue] Exécution...");
            const result = await operation();
            clearTimeout(timeoutId);
            resolve(result);
        } catch (error) {
            clearTimeout(timeoutId);
            console.error("[Queue] Erreur exécution:", error);
            reject(error);
        } finally {
...
// Fonction pour envoyer une commande à la caméra (utilise la file d'attente GATT)
async function sendCommand(characteristic, commandBuffers) {
    ...
    return gattQueue.enqueue(async () => {
        if (VERBOSE_LOGGING) console.log(`[Queue] Écriture sur UUID: ${characteristic.uuid}`);

        for (let i = 0; i < commandBuffers.length; i++) {
            const buffer = commandBuffers[i];
            try {
                await characteristic.writeValueWithResponse(buffer);
                if (VERBOSE_LOGGING && commandBuffers.length > 1) {
                    console.log(`  -> Paquet ${i + 1}/${commandBuffers.length} envoyé.`);
                }
                if (commandBuffers.length > 1 && i < commandBuffers.length - 1) {
                    await new Promise(r => setTimeout(r, 30));
                }
            } catch (error) {
                console.error(`[Queue] Échec écriture paquet ${i}:`, error);
                throw error;
            }
        }
    });
...
async function connectGopro() {
    ...
    try {
        ...
        updateConnectionStatus('Scan en cours...', 'dot-yellow');
        console.log("Demande de périphérique Bluetooth...");
        goproDevice = await BluetoothBridge.requestDevice({
...
document.addEventListener('DOMContentLoaded', async () => { // Rendre l'event listener async
    ...
    // Vérifier si Web Bluetooth ou Capacitor Bluetooth est supporté
    if (!navigator.bluetooth && !window.Capacitor) {
        displayError("Bluetooth n'est pas supporté par ce navigateur ou cette plateforme.");
